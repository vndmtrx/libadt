...
int main() {
	...
	sl_list_move_el_next(a, a->head, sl_list_next(sl_list_next(a->head)));
	print_list_iter(a);

	printf("#####  Test 5 - sl_list_move_el - Move item to tail of the list\n");
	sl_list_move_el_next(a, sl_list_next(a->head), a->tail);
	print_list_iter(a);

	printf("##### Test 6 - sl_list_move_el - Move item to another position\n");
	sl_list_move_el_next(a, sl_list_next(sl_list_next(sl_list_next(a->head))), sl_list_next(a->head));
	print_list_iter(a);

	printf("##### Test 7 - sl_list_move_el - Move head to tail (and back again)\n");
	sl_list_move_el_next(a, a->head, a->tail);
	print_list_iter(a);
	sl_list_move_el_next(a, a->tail, a->head);
	print_list_iter(a);
